...
def normalize_text(text):
    return re.sub(r"\s+", " ", clean_ocr_artifacts(text)).strip()


def prepare_semantic_text(text):
    return build_embedding_text(normalize_text(text))


def build_cluster_summary(answers):
    texts = [
        prepare_semantic_text(answer.get("raw_text", ""))
        for answer in answers
        if prepare_semantic_text(answer.get("raw_text", ""))
    ]
    return " ".join(texts)


def semantic_similarity(text_a, text_b):
    prepared_a = prepare_semantic_text(text_a)
    prepared_b = prepare_semantic_text(text_b)
    if not prepared_a or not prepared_b:
        return 0.0

    model = get_semantic_model()
    embeddings = model.encode([prepared_a, prepared_b], convert_to_tensor=True)
...
def derive_reason(best_match, required_elements):
    ...
    for element in required_elements:
        ...
        similarity = semantic_similarity(element_text, matched_text)
...
def evaluate_cluster_semantics(cluster_payload, rubric):
    semantic_variations = [
        prepare_semantic_text(text)
        for text in rubric.get("semantic_variations", [])
        if prepare_semantic_text(text)
    ]
...
def grade_cluster(cluster_payload):
    ...
    for ans in cluster_payload["answers"]:
        student_text = normalize_text(ans["raw_text"]).replace("\n", " ")
        student_semantic_text = prepare_semantic_text(student_text)
        ...
        if q_type == "theory":
            ...
            if not required:
                ...
                for concept in required:
                    similarity = semantic_similarity(concept, student_semantic_text)
